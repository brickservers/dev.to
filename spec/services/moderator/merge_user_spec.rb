...
RSpec.describe Moderator::MergeUser, type: :service do
  ...
  describe "#merge" do
    ...
    it "updates documents in Elasticsearch" do
      ...
      expect(article.search_doc.dig("_source", "user", "id")).to eq(delete_user_id)
      expect(comment.search_doc.dig("_source", "user", "id")).to eq(delete_user_id)
      expect(reaction.search_doc.dig("_source", "user_id")).to eq(delete_user_id)
      expect(article_reaction.search_doc.dig("_source", "reactable", "user", "id")).to eq(delete_user_id)

      sidekiq_perform_enqueued_jobs do
        described_class.call_merge(admin: admin, keep_user: keep_user, delete_user_id: delete_user.id)
      end
      drain_all_sidekiq_jobs
      expect(article.reload.search_doc.dig("_source", "user", "id")).to eq(keep_user.id)
      expect(comment.reload.search_doc.dig("_source", "user", "id")).to eq(keep_user.id)
      expect(reaction.reload.search_doc.dig("_source", "user_id")).to eq(keep_user.id)
      expect(article_reaction.reload.search_doc.dig("_source", "reactable", "user", "id")).to eq(keep_user.id)
    end
  end
end
